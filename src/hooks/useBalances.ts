...
export function useAssetBalances(): AssetBalances {
  ...
  const { data: BoldOnSBoldVault } = useReadContract({
    address: currentNetworkContract.sBoldToken as `0x${string}`,
    abi: sBOLD,
    functionName: 'convertToAssets',
    args: [sBoldBalance?.value || 0n],
    chainId,
  });
  ...
  const balances: AssetBalances = {
    ...
    BoldOnSBoldVault: BoldOnSBoldVaultParsed,
  };
