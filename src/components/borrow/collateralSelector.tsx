    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        <div className="flex justify-between gap-2 border-border/50 rounded-[40px] items-center">
          ...
          <span className="font-bold text-lg" >{collSymbol(selectedCollateral.symbol)}</span>
      ...
      <DropdownMenuContent className="w-60 mr-6 bg-card/10 backdrop-blur-lg border-border/50 rounded-xl">
        {COLLATERAL_TYPES.map((collateral) => (
          ...
          >
            <span className="font-semibold text-base"><img src={collateral.icon} className="w-4 h-4 mr-1 inline" /> 
              {collSymbol(collateral.symbol)}
            </span>
